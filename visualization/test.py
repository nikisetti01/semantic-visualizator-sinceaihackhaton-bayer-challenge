...
from insight_extraction.extraction.extract import (
    define_queries
)
...
def main(timestamp: str) -> None:
    ...
    excel_path = Path("datasets/data_4.xlsx")  # <-- assicurati che il path sia corretto
    ...
    # ------------------------------------------------------------------
    exec_results = define_queries(
        llm_client=sql_llm,
        allocation_path=str(assignments_path),
